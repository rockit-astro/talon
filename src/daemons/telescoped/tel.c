...
static void tel_altaz(int first, ...)
{
	if (first)
	{
		...
		toTTS(
				"The telescope is slewing towards the %s, at %.0f degrees altitude.",
...
//	}
	...
	{
		fifoWrite(Tel_Id, -5,
				"Motion controller clock drift exceeds %g sec: %g", MAXJITTER,
				x);
...
static void offsetTracking(int first, double harcsecs, double darcsecs)
{
	...
	fifoWrite(Tel_Id, 0,
			"Tracking offset by %3.3f x %3.3f arcseconds (%ld x %ld steps)",
