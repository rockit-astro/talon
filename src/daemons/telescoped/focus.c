...
static void focus_offset(int first, ...)
{
    ...
    int isworking = 0;
    if (!virtual_mode)
        isworking = csi_rix(cfd, "=working;");
    // ICE added for not to send csimc commands on virtual mode

    /* done when we reach goal */
    if ((mip->haveenc && abs(mip->raw - rawgoal) < 1 && isworking == 0) || (!mip->haveenc && mip->raw == rawgoal))
    {
        active_func = NULL;
        stopFocus(0);
...
#define NHCFG (sizeof(hcfg) / sizeof(hcfg[0]))
    ...
    static CfgEntry ocfg[] = {
        {"OAXIS", CFG_INT, &OAXIS},       {"OHAVE", CFG_INT, &OHAVE},
        {"OHASLIM", CFG_INT, &OHASLIM},   {"OPOSSIDE", CFG_INT, &OPOSSIDE},
        {"OHOMELOW", CFG_INT, &OHOMELOW}, {"OSTEP", CFG_INT, &OSTEP}, // if we have an encoder, this is encoder steps
        {"OSIGN", CFG_INT, &OSIGN},                                   // if we have an encoder, this is encoder sign
        {"OMAXVEL", CFG_DBL, &OMAXVEL},   {"OMAXACC", CFG_DBL, &OMAXACC},
        {"OSLIMACC", CFG_DBL, &OSLIMACC}, {"OSCALE", CFG_DBL, &OSCALE},
        {"OJOGF", CFG_DBL, &OJOGF},
    };
...
static char *rcsid[2] = {(char *)rcsid,
                         "@(#) $RCSfile: focus.c,v $ $Date: 2007/06/09 10:08:50 $ $Revision: 1.15 $ $Name:  $"};
