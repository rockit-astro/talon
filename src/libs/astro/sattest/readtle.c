...
int readtle(char *line1, char *line2, SatElem *elem)
{
    ...
    if (dt2[1] != '.' || (dt2[0] != ' ' && dt2[0] != '-' && dt2[0] != '+' && dt2[0] != '0'))
        fel |= 0x00000008;
    ...
    if (cospar_launch[0] == ' ' && cospar_launch[1] == ' ' && cospar_launch[2] == ' ')
        launch = 0;
    else
    {
        if (readuint(&launch, cospar_launch))
            fel |= 0x00020000;
    }

    if (cospar_piece[0] == ' ' && cospar_piece[1] == ' ' && cospar_piece[2] == ' ')
    {
        piece = 0;
    }
    else
    {
        if (readpiece(&piece, cospar_piece))
            fel |= 0x00020000;
    }

    if (readdbl(&tmp, epoch_yr))
        fel |= 0x00000004;
    if (readdbl(&(elem->se_EPOCH), epoch_frac))
        fel |= 0x00000004;
    ...
    if (readdbl(&(elem->se_XNDT20), dt2))
        fel |= 0x00000008;
    if (readfltexp(&(elem->se_XNDD60), ddt6, ddt6exp))
        fel |= 0x00000010;
    if (readfltexp(&(elem->se_BSTAR), bstar, bstarexp))
        fel |= 0x00000020;
    if (readuint(&ephtype, et))
        fel |= 0x00000040;
    if (readuint(&elnum, eln))
        fel |= 0x00000080;
    if (readflt(&(elem->se_XINCL), inclin))
        fel |= 0x00000100;
    if (readflt(&(elem->se_XNODEO), raan))
        fel |= 0x00000200;
    if (readflt(&(elem->se_EO), ecc))
        fel |= 0x00000400;
    if (readflt(&(elem->se_OMEGAO), aop))
        fel |= 0x00000800;
    if (readflt(&(elem->se_XMO), ma))
        fel |= 0x00001000;
    if (readdbl(&(elem->se_XNO), mm))
        fel |= 0x00002000;
    if (readuint(&orbit, rev))
        fel |= 0x00004000;
...
int writetle(SatElem *el, char *line1, char *line2)
{
    ...
    if (el->se_id.year)
    {
        ...
        sprintf(line1 + 9, "%02d", el->se_id.year > 2000 ? el->se_id.year - 2000 : el->se_id.year - 1900);
    }
    else
        sprintf(line1 + 9, "  ");
